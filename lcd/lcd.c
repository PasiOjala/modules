...
//    SET(LCD_RS,0);    
    SET(LCD_E,1);
//    sleep(1);  //not needed because GPIO is actually behind I2C
    SET(LCD_E,0);
}
static void write_byte(int b){
    write_nybble(b>>4); //hi
    write_nybble(b>>0);  //low
...
static void lcd_hw_init(void){
    ...
    OUTL(LCD_RS); //set at write_data
    
    // reset HD44780.pdf p.45
    //make sure adafruit is in 8-bit mode
    write_nybble(3);
    //min 4.1ms delay
    mdelay(4);
    write_nybble(3);
    write_nybble(3);
    //finally, make sure adafruit is in 4-bit mode
    write_nybble(2);
    
    write_command(0x28); //function set 2 lines, 5x8 font
...
static int ada_init(void){
  ...
  bus1 = i2c_get_adapter(1);//always works
